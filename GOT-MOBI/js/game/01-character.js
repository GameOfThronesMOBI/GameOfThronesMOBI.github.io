...
function getXpMultiplier(g) {
    ...
    if (g.brothelBuffs) {
        ...
        g.brothelBuffs.forEach(function(buff) {
            if (buff.type === 'xp') multiplier += buff.value / 100;
        });
    }
...
function getWeaponMasteryBonus(g) {
    ...
    if (!weapon) {
        return { damageBonus: 0, defenseBonus: 0, agilityBonus: 0, critBonus: 0, pierceBonus: 0, doubleHitBonus: 0, counterBonus: 0, speedBonus: 0, skillLevel: 0, weaponType: 'нет' };
    }
    ...
    return { damageBonus: damageBonus, defenseBonus: defenseBonus, agilityBonus: agilityBonus, critBonus: critBonus, pierceBonus: pierceBonus, doubleHitBonus: doubleHitBonus, counterBonus: counterBonus, speedBonus: speedBonus, skillLevel: skillLevel, weaponType: weaponType };
}
...
function distributeMasteryPoint(type) {
    ...
    var g = user.game;
    if (g.activeBonuses.points <= 0) { setMessage('❌ Нет доступных очков мастерства.'); return; }
    var current = g.activeBonuses[type] || 0;
    if (current >= 65) { setMessage('❌ Максимум для ' + type + ' достигнут.'); return; }
...
function getTotalStats(g) {
    ...
    if (g.equipment && g.equipment.boots && g.equipment.boots.speedPercent) speedPercent += g.equipment.boots.speedPercent;
    if (g.equipment && g.equipment.horse) {
        var horse = HORSE_TYPES[g.equipment.horse.horseType];
    ...
    if (horseDefensePercent > 0) finalDefense = Math.round(finalDefense * (1 + horseDefensePercent / 100));
    var luck = Math.min(25, g.luck || 0);
    var critFromLuck = Math.floor(luck / 10);
    var blessingMultiplier = 1;
    if (g.blessing && g.blessing.active && g.blessing.expires > Date.now()) blessingMultiplier = 1.1;
    var damageBuff = 0;
    if (g.brothelBuffs) {
        var now = Date.now();
        g.brothelBuffs = g.brothelBuffs.filter(function(b) { return b.expires > now; });
        g.brothelBuffs.forEach(function(buff) {
            if (buff.type === 'damage') damageBuff += buff.value;
        });
    }
    return {
...
function addAttribute(statId) {
    ...
    if (!user) { setMessage('❌ Игрок не найден.'); return; }
    var g = user.game;
    if (g.attributePoints <= 0) { setMessage('❌ Нет свободных очков атрибутов.'); return; }
    g.attributePoints--;
...
function openCharacter() {
    ...
    var statLabels = { damage: '⚔️ Урон', defense: '🛡️ Защита', intelligence: '🧠 Интеллект', agility: '💨 Ловкость' };
    ['damage', 'defense', 'intelligence', 'agility'].forEach(function(s) {
        var base = g.stats[s] || 1;
        var bonus = 0;
        var bonusText = '';
        if (s === 'damage') { bonus = equipped.bonusDamage + mastery.damageBonus; bonusText = ' (оружие: +' + equipped.bonusDamage + ' | мастерство: +' + mastery.damageBonus + ')'; }
        if (s === 'defense') { bonus = equipped.bonusDefense + mastery.defenseBonus; bonusText = ' (броня: +' + equipped.bonusDefense + ' | мастерство: +' + mastery.defenseBonus + ')'; }
        if (s === 'agility') { bonus = equipped.bonusAgility + mastery.agilityBonus; bonusText = ' (экипировка: +' + equipped.bonusAgility + ' | мастерство: +' + mastery.agilityBonus + ')'; }
